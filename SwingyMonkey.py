...
class SwingyMonkey:
    ...
    def __init__(self, sound=True, text=None, action_callback=None, 
        ...
        self.background_img = pg.image.load('assets/visual/jungle-pixel.bmp').convert()
        self.monkey_img     = pg.image.load('assets/visual/monkey.bmp').convert_alpha()
        self.tree_img       = pg.image.load('assets/visual/tree-pixel.bmp').convert_alpha()
        if self.sound:
            self.screech_snd    = pg.mixer.Sound('assets/audio/screech.wav')
            self.blop_snd       = pg.mixer.Sound('assets/audio/blop.wav')
